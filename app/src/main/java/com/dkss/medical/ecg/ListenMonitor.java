...
public class ListenMonitor implements Runnable{
    private static final int PAYLOAD_LIMIT = 2042;
    private ArrayList<byte[]> bufferDataQueue = new ArrayList<>();
    private  int bufferDataQueueLen = 50;

    private ArrayList<Short> remainList = new ArrayList<>();
    public volatile String mIP = "";
    public volatile boolean isData = false;

    private static byte[] boxIDPayload = DkssUtil.constructPayload(new byte[] { 0x00, 0x01 },"c3653127-08b3-488d-9b80-77b7e6082910");
    private static byte[] boxTypePayload = DkssUtil.constructPayload(new byte[] {0x00,0x04},"medicalbox");
    private static byte[] devTypePayload = DkssUtil.constructPayload(new byte[]{0x00,0x05},"pmr");
